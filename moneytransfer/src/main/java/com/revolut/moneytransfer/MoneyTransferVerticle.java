...
public class MoneyTransferVerticle extends AbstractVerticle {
	...
	//Used in-memory datastore. Map will store the data inside the volatile memory i.e. RAM.
	//Once the application is terminated all the data will be gone.
	//I could have used some third party jar like H2 for in-memory database, but Map could be a good option in this case.
	...
	//The application will start from here, runs a Vert.x instance..
	...
	public void start(Future<Void> fut) {
		...
		// Will create some data on the application start. Mainly Used for the slowTest.
		createSomeData();
	...
	private void updateTransfer(RoutingContext routingContext) {
		...
		if (id == null) {
			...
			if (transfer == null) {
				...
				if (transfer.getStatus() != Transfer.TransferStatus.EXECUTED
						&& transfer.getStatus() != Transfer.TransferStatus.FAILED
						&& transfer.getAmount().compareTo(BigDecimal.ZERO) > 0
						&& accounts.get(transfer.getSourceAccountId()) != null
						&& accounts.get(transfer.getDestinationAccountId()) != null
						&& accounts.get(transfer.getSourceAccountId()).getBalance().compareTo(transfer.getAmount()) >= 0
						&& accounts.get(transfer.getSourceAccountId()).getCurrency()
								.equals(accounts.get(transfer.getDestinationAccountId()).getCurrency())
	...
	private void createSomeData() {
		Account acc1 = new Account("Nelson Mandela", new BigDecimal("2345"), Currency.getInstance("EUR"));
		accounts.put(acc1.getId(), acc1);
		Account acc2 = new Account("Albert Einstein", new BigDecimal("437"), Currency.getInstance("EUR"));
		accounts.put(acc2.getId(), acc2);
		Account acc3 = new Account("Usain Bolt", new BigDecimal("10000"), Currency.getInstance("GBP"));
